...
\begin{document}
	...
	\begin{poster}{
		...
		\headerbox{\textbf{1) Introducción}}{name=intro,column=0,below=abstract,span=2}{\footnotesize 
			...
			\vspace{-5mm}
			\begin{center}
				\includegraphics[width=8cm]{temysem.pdf}
			\end{center}
					{\begin{singlespace}
							\tiny
							\textbf{Fig. 1:}
							Imágenes nanopartículas de plata con diferentes formas tomadas en un microscopio electrónico de transmisión (TEM): \textbf{(a)} nanoesferas, \textbf{(b)} nanobarras, \textbf{(c)} nanofibras y en un microscopio electrónico de barrido (SEM): \textbf{(d)} pirámides, \textbf{(e)} nanocubos, \textbf{(f)} nanoflores. Tomada y adaptada de \cite{Mendez}.
						\end{singlespace}
		...
		\headerbox{ \textbf{2) Teoría de Mie}}{name=MieTh,column=0,below=intro,span=2}{\footnotesize 
			...
			\begin{minipage}[c]{.55\linewidth}
				{\footnotesize
					...
					con $\psi_{\ell}(\rho)=\rho j_{\ell}(\rho)$ y $\xi_{\ell}(\rho)=\rho h_{\ell}^{(1)}(\rho)$ las funciones de Ricatti-Bessel, dadas en términos de las funciones esféricas de Bessel $j_{\ell}$ y las funciones esféricas de Hankel de primer tipo $h_{\ell}^{(1)}$,  $m=n_p/n_m$  y $x=2 \pi n_m/a$ el parámetro de}
			\end{minipage}
			\begin{minipage}[c]{.45\linewidth}
				%\centering
				\includegraphics[width=5cm]{scattering.png}
				\begin{singlespace}
					\tiny \textbf{Fig. 2:} Partícula de radio $a$ e índice de refracción $n_p$ inmersa en un medio de índice refracción $n_m.$
				\end{singlespace}
				\label{fig:sample_figure}
			\end{minipage}	\\
			tamaño. La {\color{mxpink}\textbf{sección transversal de extinción}} se escribe en términos de los coeficientes de Mie como \cite{Bohren}:
			...
			donde $\text{Re}\{\cdot\}$ representa la parte real de un número complejo. Su máximo corresponde a la excitación del LSPR \cite{Jonathan}, que se obtiene cuando el denominador  de los coeficientes de Mie es mínimo \cite{Novotny}.\\
		...
		\headerbox{\textbf{3) Esferas plasmónicas en el límite $k_m a \ll 1$
			...
			La respuesta electromagnética de un material puede describirse mediante su índice de refracción. Este puede estar dado en términos del  {\color{mxpink}\textbf{modelo de Drude}}, que representa la solución de la ecuación de movimiento de los electrones libres de un material en presencia de un campo eléctrico oscilante  \cite{Novotny, Ashcroft}, el cual está dado por: 
		...
		\headerbox{\textbf{4) Resultados numéricos}}{name=nmodo,span=2,column=2,below=abstract}{\footnotesize
			...
			\vspace{2mm}
			Se graficaron las secciones transversales de extinción $C_{ext}$ en función de $\omega/\omega_p$ para los primeros cinco multipolos para distintos valores de $a\omega_p /c$ [Ver ejemplo en Fig. 3(a) y 3(b)]. Mediante el método de la sección dorada, se encontraron las frecuencias $\omega_l$ que maximizan la sección transversal de extinción y se graficó $\omega_l/\omega_p$ en función de $a\omega_p /c$ [Ver Fig. 3(c)].\\
						
			\begin{tikzpicture}
				\node[inner sep=0pt] (graf) at (0,5.7){\includegraphics[scale=.53]{labels.pdf}};
				\node at (-2.5,5) {\footnotesize \textbf{Multipolos eléctricos}};
				\node at (3,5) {\footnotesize \textbf{Multipolos magnéticos}};
				\node[inner sep=0pt] (graf) at (0,-0.2){\includegraphics[scale=.43]{results.pdf}};
			\end{tikzpicture}
				\begin{singlespace}
					\tiny\textbf{Fig. 3:}  Sección transvesal de extinción en función del cociente entre la frecuencia de excitación y la frecuencia de plasma para \textbf{(a)} $a\omega_p /c$=1.5 y \textbf{(b)} $a\omega_p /c$=5. \textbf{(c)} Frecuencia de excitación entre la frecuencia de plasma como función del parámetro adimensional $a\omega_p /c$. En la gráfica de la derecha se observan los valores a los que tiende la función al reducir el tamaño de la partícula.
				\end{singlespace}
		...
		\headerbox{\textbf{5) Conclusiones}}{name=conclusiones,column=2,below=nmodo,span=2}{
			\footnotesize
			\begin{itemize}
				\item[\textcolor{mxpink}{\ding{212}}] Para partículas esféricas conforme el límite de partícula pequeña deja de ser válido, la energía de los modos eléctricos y magnéticos se reduce, por lo que se presenta un corrimiento al rojo de las SPRs.
