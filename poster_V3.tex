...
\begin{document}
	...
	\begin{poster}{
		...
		\headerbox{ \textbf{2) Teoría de Mie}}{name=MieTh,column=0,below=intro,span=2}{\footnotesize 
			...
			\end{minipage}	\\
		 funciones esféricas de Hankel de primer tipo $h_{\ell}^{(1)}$,  $m=\sqrt{\epsilon_p/\epsilon_m}$  y $x=2 \pi n_m/a$ el parámetro de tamaño. La {\color{mxpink}\textbf{sección transversal de extinción}} está dada en términos de los coeficientes de Mie como \cite{Bohren}:
		...
		\headerbox{\textbf{3) Esferas plasmónicas en el límite $k_m a \ll 1$
			...
			La respuesta electromagnética de un metal puede describirse mediante el  {\color{mxpink}\textbf{modelo de Drude}}, el cual considera las interacción entre los electrones libres de un material  y un campo eléctrico armónico, proporcionando la función dieléctrica $\epsilon_p(\omega)$ para un metal \cite{Novotny}: 
			...
			\end{minipage}
				\begin{minipage}[c]{0.42\textwidth}
					\begin{tikzpicture}
						\node at (-1.5,6.5) {$\omega_p$: frecuencia de plasma};
						\node at (-0.9,6.2) {	$\gamma$: constante fenomenológica de };
						\node at (-1.45,5.9) {amortiguamiento};
					\end{tikzpicture}
			\end{minipage}			
			...
			\begin{minipage}[c]{.55\linewidth}
				...
				$$\tcboxmath[colback=orange!15!white ,colframe=orange,size=title]{\frac{\omega_\ell}{\omega_p}=\sqrt{\frac{\ell}{2\ell+1}}.}$$
					...
					\begin{tikzpicture}
					\node at (-1.5,7) {\scriptsize{\color{orange} Dipolo eléctrico}};
					\node at (1.5,7) {\footnotesize $\omega_1/\omega_p=1/\sqrt{3}$};
					\node at (-1.5,6.5) {\scriptsize{\color{orange} Plasmón de superficie}};
					\node at (-1.4,6.2) {\scriptsize{\color{orange} de superficie}};
					\node at (1.45,6.5) {\footnotesize $\omega_{\infty}/\omega_p=1/\sqrt{2}$};					
				\end{tikzpicture}
			\end{minipage}
			\raisebox{6.9ex}{
